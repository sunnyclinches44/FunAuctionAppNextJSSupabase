...
interface SessionState {
  ...
  undoBid: (participantId: string) => void
  ...
  devtools(
    subscribeWithSelector((set, get) => ({
      ...
      addBid: (bid) => set((state) => {
        ...
        if (participant) {
          // Participant amounts are already updated by database - don't double-add
          // Just add the bid to the bids array
      ...
      undoBid: (participantId: string) => set((state) => {
        // Find the last bid for this participant
        const lastBid = state.bids.find(bid => bid.participant_id === participantId)
        
        if (lastBid) {
          // Remove the last bid
          const updatedBids = state.bids.filter(bid => bid.id !== lastBid.id)
          
          // Update participant amount
          const updatedParticipants = state.participants.map(p => 
            p.id === participantId 
              ? { ...p, amount: Math.max(0, Number(p.amount || 0) - Number(lastBid.delta || 0)) }
              : p
          )
      ...
      advanceRound: async (sessionCode, round) => {
        try {
          ...
          return false
            ...
            undoBidAsync: async (sessionCode: string, deviceId: string) => {
        try {
          const { data, error } = await supabase.rpc('undo_last_bid', {
            p_session_code: sessionCode,
            p_device_id: deviceId
          })
          ...
          if (data && data.success) {
            // Find the participant by device_id
            const participant = get().participants.find(p => p.device_id === deviceId)
            if (participant) {
              // Update the participant's amount
              set((state) => ({
                participants: state.participants.map(p => 
                  p.device_id === deviceId 
                    ? { ...p, amount: data.new_total }
                    : p
                ),
                totalAmount: state.participants.reduce((sum, p) => 
                  p.device_id === deviceId 
                    ? sum - (participant.amount - data.new_total)
